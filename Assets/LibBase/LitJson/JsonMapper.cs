...
namespace LitJson
{
    ...
    public class JsonMapper
    {
        ...
        public static ILitJsonTypeHelper Helper;
        
        private static object ReadValue (Type inst_type, JsonReader reader, string varKey = "")
        {
            reader.Read ();
            ...
            if (reader.Token == JsonToken.Double ||
                ...
                // No luck
                throw new JsonException (String.Format (
                        "Can't assign {0} value '{1}' (type {2}) to type {3}",
                        varKey, reader.Value, json_type, inst_type));
            }
            ...
            if (reader.Token == JsonToken.ArrayStart) {
                ...
                if (! t_data.IsArray && ! t_data.IsList)
                    throw new JsonException (String.Format (
                            "{0} Type {1} can't act as an array",
                            varKey, inst_type));
                ...
                while (true) {
                    object item = ReadValue (elem_type, reader, varKey);
            ...
            {
                if (TypeIsPolymorphismJsonAttribute(value_type))
                {
                    reader.Read(); // __TYPE__
                    reader.Read();
                    string type = (string) reader.Value;
                    Type t = Helper.GetType(type);
                    if (t == null)
                    {
                        value_type = null;
                    }
                ...
                if (value_type != null)
                {
                    ...
                    isIntKey = t_data.IsDictionary && value_type.GetGenericArguments()[0] == typeof(int);
                }
                else
                {
                    t_data = new ObjectMetadata();
                    t_data.Properties = new Dictionary<string, PropertyMetadata>();
                }
                ...
                while (true)
                {
                    ...
                    if (t_data.Properties.ContainsKey(key))
                    {
                        ...
                        if (prop_data.IsField)
                        {
                            ...
                            if (TypeIsPolymorphismJsonAttribute(prop_data.Type))
                            {
                                ...
                                ((FieldInfo)prop_data.Info).SetValue(instance, ReadValue(t, reader, prop_data.Info.Name));
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            if (p_info.CanWrite)
                                p_info.SetValue(
                                    instance,
                                    ReadValue(prop_data.Type, reader, prop_data.Info.Name),
                                    null);
                            else
                                ReadValue(prop_data.Type, reader, prop_data.Info.Name);
                        }
                    ...
                    else
                    {
                        if (!t_data.IsDictionary)
                        {
                            ...
                            {
                                throw new JsonException(String.Format(
                                    "The {0} type {1} doesn't have the " +
                                    "property '{2}'",
                                    varKey, inst_type, key));
                            }
                            else
                            {
                                ReadSkip(reader);
                        ...
                        if (isIntKey)
                        {
                            ...
                            if (convertible == null)
                            {
                                ...
                                else if (dictValueType == typeof(int))
                                {
                                    var v = Convert.ChangeType(readValue.ToString(), dictValueType);
                                    dict.Add(Convert.ToInt32(key), v);
                                }
                                ...
                                {
                                    dict.Add(Convert.ToInt32(key), readValue.ToString());
                                }
                                else
                                {
        ...
        private static void RegisterBaseExporters ()
        {
            ...
            base_exporters_table[typeof(float)] = delegate(object obj, JsonWriter writer) { writer.Write((float)obj); };
        }
                                        ...
                                        int depth)
        {
            ...
            foreach (PropertyMetadata p_data in props) {
                ...
                if (skipAttributes != null && skipAttributes.Count > 0)
                {
                    continue;
                }
                if (p_data.IsField) {
        ...
        private static void WriteValueWithoutNull (object obj, JsonWriter writer,
                                        bool writer_is_private,
                                        int depth)
        {
            if (depth > max_nesting_depth)
            ...
            if (obj is Array) {
                ...
                foreach (object elem in (Array) obj)
                    WriteValueWithoutNull (elem, writer, writer_is_private, depth + 1);
            ...
            if (obj is IDictionary) {
                ...
                foreach (DictionaryEntry entry in (IDictionary) obj) {
                    writer.WritePropertyName (entry.Key.ToString());
                    WriteValueWithoutNull (entry.Value, writer, writer_is_private,
                                depth + 1);
                }
                writer.WriteObjectEnd ();
            ...
            foreach (PropertyMetadata p_data in props) {
                ...
                if (p_data.IsField)
                {
                    object childObj = ((FieldInfo)p_data.Info).GetValue(obj);
                    if (childObj != null)
                    {
                        writer.WritePropertyName (p_data.Info.Name);
                        WriteValueWithoutNull (((FieldInfo) p_data.Info).GetValue (obj),
                            writer, writer_is_private, depth + 1);
                    }
                }
                else {
                    PropertyInfo p_info = (PropertyInfo) p_data.Info;
                    object childObj = p_info.GetValue (obj, null);
                    if (childObj != null)
                    {
                        if (p_info.CanRead) {
                            writer.WritePropertyName (p_data.Info.Name);
                            WriteValueWithoutNull (p_info.GetValue (obj, null),
                                writer, writer_is_private, depth + 1);
        ...
        public static string ToJsonWithoutNull(object obj)
        {
            lock (static_writer_lock) {
                ...
                WriteValueWithoutNull(obj, static_writer, true, 0);
